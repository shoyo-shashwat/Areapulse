...
def analyze():
    ...
    try:
        ...
        if not parsed.get("issues"):
            print("[WARN] Empty issues array, retrying once for a non-empty result", flush=True)
            retry_parsed, retry_raw, _ = call_groq_vision(image_b64)
            if retry_parsed and retry_parsed.get("issues"):
